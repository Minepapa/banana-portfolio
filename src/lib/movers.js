...
// 실제 내 계좌가 얼마 변했나를, 종목 무버는 "단가 변동만"으로 순수 가격 움직임을 보여준다.
...
export function computeDailyChange(accounts, snapshot, { topN = 5 } = {}) {
  ...
  // 변동 0원(예: 기준선을 방금 찍은 직후)은 무버가 아니므로 제외 → 의미 없는 ₩0 목록 방지.
  const ranked = movers.filter(m => m.wonDelta !== 0).sort((a, b) => Math.abs(b.wonDelta) - Math.abs(a.wonDelta));
