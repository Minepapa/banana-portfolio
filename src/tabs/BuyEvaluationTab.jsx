...
export default function BuyEvaluationTab({
  ...
  // 초기값을 null로(=targetNonce로 하면 안 됨): 이 탭은 tab==="평가"일 때만 마운트되는
  // 조건부 렌더라 리스크탭→평가탭 이동 시 항상 새로 마운트되는데, useState(targetNonce)로
  // 초기화하면 마운트 첫 렌더에서 이미 "같음"으로 판정돼 동기화가 발동하지 않는 버그가 있었음
  // (실측: 현대차·하이닉스 "평가 보기" 클릭 시 항상 evalSelectedIdx 기본값=삼성전자로 남음).
