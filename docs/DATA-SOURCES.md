...
### 확정된 한계
- **업종분류(섹터) API 없음** — 전체 카탈로그(지수/주식/증권상품/채권/파생상품/일반상품/ESG) 어디에도 종목별 업종코드(GICS/WICS류)를 주는 서비스가 없음. `종목기본정보`엔 시장구분(KOSPI/KOSDAQ)·소속부(벤처기업부 등)만 있고 진짜 업종분류는 없음. ⚠️ **KRX 기준 한계일 뿐** — KIS `search-stock-info`(tr_id `CTPF1002R`)의 `idx_bztp_mcls_cd_name`/`idx_bztp_scls_cd_name` 필드로 우회 가능(단, 코스피 종목만 채워짐 — 코스닥은 빈 문자열). 2026-08-20 실측 확인, 아직 미배선.
- **지수 구성종목(멤버십) API 없음** — "KOSPI200에 어떤 종목이 속하는가" 같은 조회가 카탈로그에 없음. 지수 카테고리는 지수 자체의 시세(레벨값)만 제공. ⚠️ **KRX 기준 한계일 뿐** — KIS `search-stock-info`의 `kospi200_item_yn` 필드로 종목별 정확한 코스피200 편입여부 확인 가능(코스닥150 전용 필드는 없음, 여전히 미해결). 2026-08-20 실측 확인, 아직 미배선. `etp/etf_bydd_trd`·KIS ETF 구성종목시세(`FHKST121600C0`)로 KODEX200 등의 PDF도 일부 가능하나 상위 30건 캡 있어 전체 대체는 불가.
- **실시간·계좌·주문 기능 없음** — 전부 일별 배치(`basDd` 파라미터) 데이터. 실시간 시세·잔고조회·주문 실행은 KIS API 영역.
- **국내 시장 전용** — 미국 주식·환율·금리 등 해외 데이터 없음. KIS도 마찬가지(overseas_stock 카테고리 전수 확인, 해외종목 시세류만 있고 거시지표 전용 API 없음, 2026-08-20 확인) — yfinance 의존 유지.
...
**ECOS로 다른 yfinance 지표를 대체하는 것은 조사 후 대부분 기각**(2026-09-12,
오너 지시 "ECOS로 대체 가능한 지표는 1순위로" 이행 중 확인): 미국 10년물 금리
(902Y023)·해외 주가지수(902Y002)·국제상품가격(WTI·금 등, 902Y003)은 ECOS에
있지만 **월/분기/연단위만 제공**돼 이 프로젝트의 일단위 신호(Faber 200일 이평·
±2σ 볼린저)와 안 맞아 대체 불가로 판정. 원/달러 환율(731Y001/0000001)은
일단위로 있지만 **전영업일 지연**(오늘 발행값이 어제 시장 반영)이 있어 risk-
monitor 거시신호·실시간 포지션평가 둘 다에 부적합 — 실제 교체를 구현까지
했다가 code-reviewer 실측으로 지연을 발견해 yfinance로 되돌림(상세는
`Knowledge/API/ECOS.md` 참고). 국고채 스프레드(위)만 유일하게 일단위+무지연
조건을 만족해 실제 대체됨.
