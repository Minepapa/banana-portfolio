...
2. 프롬프트에 위 지시 내용과 함께 직접 보고 모드 절차를 포함한다: memory/athena.md 읽기 → 소관 데이터 직접 조회(종목투자노트·평가요청·시세, 출처 표기) → 아테나의 신화적 성격대로 서술형 보고(강제 첫 문장 없음 — 성격은 athena.md 정본).
2-1. **반(反)밋밋 게이트**(스폰 프롬프트에 명시): 일반론 답변 금지. 반드시 (a) memory/athena.md 실제 읽기, (b) 소관 시트·시세를 **최소 2건 이상 실제 조회**해 수치를 인용(출처 표기), (c) 못 구하면 "(데이터 부족: 소스)"로 명시 — 조회 없이 추상적으로 답하면 실패다.
