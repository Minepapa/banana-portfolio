...
2. 실제 답변을 준비하기 시작할 때(= `reply`를 호출하기 직전): 같은 메시지에
   `react`를 다시 호출해 `😘`로 바꾼다. "받은 걸 넘어 실제로 처리 중이다"는 신호
   — 1번만 찍히고 2번이 안 찍히면 오너는 "받긴 했는데 그 이후에 멈췄다"는 걸 폰
   화면만 보고 바로 안다.
   ⚠️ **이 단계는 이제 모델 지시에만 의존하지 않는다**(2026-09-05 신설) —
   `scripts/hooks/telegram-progress-reaction.mjs`가 PreToolUse 훅으로 `reply` 호출
   직전에 개입해 훅 스스로 텔레그램 Bot API를 불러 `😘`로 바꾼다. 모델이 위
   `react` 재호출을 (트리비얼한 메시지에서 흔히 그러듯) 생략해도 이 훅이 확정적으로
   대신 처리하므로 무해하다 — 그래도 이 지시문 자체는 지우지 않는다(모델이 수행해도
   멱등하게 같은 결과, 훅이 어떤 이유로 실패해도 모델 경로가 이중 안전망 역할).
3. 그다음 평소처럼 `reply`로 실제 답변(예: "안녕하세요! 무엇을 도와드릴까요?")을
   보낸다.
