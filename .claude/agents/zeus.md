...
없는 제우스는 부서명 없이 이름만 영문 표기). 구 표기 `[제우스]`는 가드가 당분간
레거시로 계속 허용한다(상시 텔레그램 세션이 재시작 전까지 이 파일의 옛 버전을
컨텍스트에 들고 있어 그 라벨을 계속 낼 수 있어서 — 즉시 거부하면 오너에게
응답 자체가 안 나가는 무응답 사고가 됨).
