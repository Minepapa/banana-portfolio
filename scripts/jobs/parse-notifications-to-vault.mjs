 ...
 *
 * ⚠️ 범위(2026-08-04, 구현계획서 Phase 2 — 오너 확정): 오늘은 **체결·배당만** Vault에
 * 기록한다. 펀드적립·금현물·예수금앵커·환전은 파서는 이미 준비돼 있지만(notification-
 * parsers.mjs), 계좌 귀속·다계좌 회계·보유종목 갱신은 State/Holdings 설계가 끝나는
 * Phase 8·9에서 마저 연결한다 — 지금 억지로 끼워 넣지 않는다(설계 없이 만들면 나중에
 * 다시 뜯어고쳐야 함).
 *
...
import { parseExecution, parseDividend } from '../lib/notification-parsers.mjs';
...
async function main() {
  ...
  if (!DRY_RUN) mkdirSync(VAULT_PATHS.facts.ledger, { recursive: true });
  ...
  for (const r of alarmRows) {
    ...
    if (e) {
      const { filename, content } = buildExecutionRecord(e);
      const filepath = join(VAULT_PATHS.facts.ledger, filename);
    ...
    if (d) {
      const { filename, content } = buildDividendRecord(d);
      const filepath = join(VAULT_PATHS.facts.ledger, filename);
      if (existsSync(filepath)) { skip++; continue; }
    ...
    // 체결·배당이 아닌 다른 5종(펀드적립·금현물·예수금앵커·환전)일 수도, 파싱 대상이
    // 아예 아닌 알림(광고 등)일 수도 있다 — 지금 범위 밖이라 개수만 집계하고 넘어간다.
    unrecognized++;
  }

  console.log(
    `\n✅ 완료 — 체결 +${execNew} · 배당 +${divNew} · 중복스킵 ${skip} · 범위밖/미인식 ${unrecognized}` +
    (DRY_RUN ? ' (드라이런 — 쓰기 없음)' : ''),
  );
}
