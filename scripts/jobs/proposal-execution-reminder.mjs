 ...
 *
 * ①**미체결 리마인더** — 위탁·금현물은 승인 후 execute-asset-allocation.mjs가 NH PLUG
 * 주문을 자동 제출하지만 주문 접수가 실제 체결을 보장하지는 않는다. 연금저축은 자동
 * 실행 대상이 아니므로 오너가 직접 브로커 앱에서 주문한다. 이 잡은 승인 후 미체결
 * 상태가 오래 지속되는 제안(특히 연금저축)을 잊지 않도록 리마인드한다.
 *
