#!/usr/bin/env node
/**
 * 체결모드(섀도우|실전) 전환 — Phase 12. Zeus(상시 세션)가 Frank의 "체결모드 온"/
 * "체결모드 오프" 텍스트를 받으면 이 CLI를 호출한다. State/ExecutionMode/
 * ExecutionMode.md를 갱신한다 — 킬스위치 전환(kill-switch-cli.mjs)과 동일한 패턴.
 * (구 "실전전환"/"섀도우전환" → 2026-09-18 "실전모드 온"/"실전모드 오프"로 재통일 →
 * 2026-09-19 스위치 이름(체결모드)과 명령어 접두어가 어긋난다는 지적으로 "체결모드
 * 온"/"체결모드 오프"로 재개명, 오너 지시. 구 명령어는 하위호환 없이 폐기.)
 *
 ...
 *
 * 사용법: node scripts/tools/execution-mode-cli.mjs --text="체결모드 온"
 */
...
async function main() {
  ...
  if (!cmd) {
    console.log(`ℹ️ "${text}"는 체결모드 전환 명령이 아닙니다(정확히 "체결모드 온"/"체결모드 오프"만 인정) — 무시`);
    return;
  }
  ...
  if (targetMode === MODE_LIVE) {
    console.log('🔴 체결모드 온(실전) — 검문소를 통과한 승인된 제안은 이제 실제 KIS 계좌에 매수/매도 주문이 나갑니다(카이로스는 이 스위치와 무관하게 항상 실전). 되돌리려면 "체결모드 오프" 명령.');
  } else {
    console.log('🟢 체결모드 오프(섀도우) — 이후 체결은 실제 주문 없이 로그로만 기록됩니다.');
  }
}
