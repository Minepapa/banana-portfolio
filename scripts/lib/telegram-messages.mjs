...
// 때 공통으로 쓰는 마커. 2026-09-14엔 "[Zeus]"(영문)와 "[제우스]"(국문) 두 표기가
// 같은 화자를 가리키며 공존하던 걸 "[제우스]"로 통일했었으나, 2026-09-19 오너
// 재지시로 다시 "[Zeus]"(영문)로 환원 — 부서 라벨이 전부 "부서명(국문)+이름(영문)"
// 형식(운영실 Hermes 등)인데 제우스는 부서가 없어 이름만 영문으로 표기하는 게
// 형식에 맞다는 판단(DevRequest 2026-09-19-제우스-라벨-영문표기-Zeus.md). 구 표기
// "[제우스]"는 telegram-sender-label-guard.mjs가 당분간 레거시로 계속 허용한다
// (상시 텔레그램 세션이 재시작 전까지 구 지시문으로 이 라벨을 계속 낼 수 있어서 —
// 그 가드가 즉시 거부하면 오너에게 응답 자체가 안 나가는 사고가 됨).
