...
// 실사고 회귀 테스트(2026-08-19, 오너 제보) — NH 분배금 입금 안내 실제 원문 그대로.
// "계좌번호" 키워드 없이 "{마스킹번호} {이름}님 계좌로" 형태로 계좌번호가 있는데,
// 예전엔 이 형식을 못 잡아 acctRaw가 항상 빈 문자열이었다(계좌귀속 실패의 근본원인).
test('parseDividend: NH 분배금 입금 안내 실제 형식 — acctRaw가 대시 포함 형태로 정확히 채워짐', () => {
  const body = '[NH투자증권] 분배금 입금 안내\n2090289***2 정*호 님 계좌로 분배금 입금 안내 드립니다. \n\n1. 종목명 : 미래에셋 TIGER 미국배당다우존스타겟데일리커버드콜증권상장지  \n2. 기준일 : 2026.08.14\n3. 세전금액 : 64,890원\n  세후금액 :64,890원\n  세금합계 : 0원\n4. 입금일: 2026.08.19 ';
  const r = parseDividend(body, '2026-08-19 10:30:31');
  assert.equal(r.afterTaxAmount, 64890);
  assert.equal(r.stockName, '미래에셋 TIGER 미국배당다우존스타겟데일리커버드콜증권상장지');
  assert.equal(r.acctRaw, '209-02-89***2');
});
