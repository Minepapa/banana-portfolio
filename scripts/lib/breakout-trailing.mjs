...
import { computeTrailingStop } from './breakout-risk.mjs';

// position: breakout-position-vault.mjs가 만든 레코드(parseBreakoutPosition 결과) —
// entryPrice·highSinceEntry·stopPrice 필요. latestHigh: 오늘까지 확인된 최신 고가
// (당일 장중 고가 또는 전일 일봉 고가 — 호출측이 어느 걸 쓸지 결정).
//
...
export function computeTrailingRevision(position, latestHigh) {
  ...
  const newStopPrice = computeTrailingStop(position.entryPrice, newHighSinceEntry);
