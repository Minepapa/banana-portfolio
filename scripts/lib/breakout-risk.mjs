...
// TIGHT_STOP_LOSS_PCT(4%)를 쓴다.
//
// 8.0으로 재조정(2026-09-18 백테스트, 2026-09-19 오너 확정 실전배선) — 초기값
// 4.0은 "TIGHT_STOP_LOSS_PCT와 정확히 같은 수치"로 맞춘 것일 뿐 근거가 없었고,
// 실제 백테스트(2014~2026, 거래량확인+RS≥8 결합)에서 오히려 baseline(ATR 없음)
// 보다 성과가 나빴다(샤프 0.66→0.48) — 종목의 약 31%(임계값 4.0 기준)가
// "좁은손절+Max2%룰 반비례 사이징으로 2배 투입"을 동시에 받아 분산이 무너진
// 게 원인으로 추정됐다.
//
// ⚠️ 인과관계 정정(2026-09-19 코드리뷰 HIGH 지적 — 최초 주석이 사실과 반대로
// 적혀 있었음) — "임계값을 올리면 좁은손절+2배사이징 비율이 줄어든다"는 최초
// 가설은 **틀렸다**. selectAdaptiveStopLossPct는 atrPct<thresholdPct일 때
// tight(4%)를 주므로, 임계값을 올릴수록 그 조건을 만족하는 종목은 **늘어난다**
// — 실측(캐시 실데이터, 52주신고가 신호일 n=41,028): 임계값 4.0에서 tight
// 비율 39.5% → 임계값 8.0에서 **84.6%**로 오히려 2배 이상 증가. 그런데도
// 8.0의 백테스트 성과(연환산 13.0%·샤프0.74, baseline 9.7%·0.66보다 우수)는
// 실측 사실 그대로다 — 진짜 원인은 돌파신호 종목 자체가 VCP(변동성수축)
// 패턴이라 애초에 ATR%가 낮은 경우가 대부분이라는 것: 임계값 8.0은 "종목별로
// 4%/8%를 적절히 섞는 전략"이 아니라 **"돌파신호 대부분(≈85%)에 4%+2배
// 사이징을 적용하고, 드물게 진짜 변동성 큰 종목만 8%로 완화"에 가까운 동작**
// 이다. MAX_CONCURRENT_POSITIONS=10 슬롯 분산 전제가 실질적으로 무의미해짐
// (첫 진입이 예수금의 50%를 먹어 실질 2~3종목에 집중)을 뜻하므로, 이 특성을
// 오너가 인지하고 있는지 별도 확인 필요.
// 4%(연환산 8.8%·샤프0.60) < 6%(9.8%·0.61) < 8%(13.0%·0.74) ≈ 10%(13.5%·0.74),
// 8%가 10%와 거의 동률이면서 더 보수적이라 8%로 확정
// (Log/Implementation/2026-09-19-카이로스점검-이스케이프-runsh인자-ATR재조정.md
// 참고).
export const ATR_STOP_THRESHOLD_PCT = 8.0;
